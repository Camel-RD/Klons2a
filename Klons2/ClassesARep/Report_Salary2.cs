...
namespace KlonsA.Classes
{
    public class Report_Salary2
    {
            ...
            int? idp, int? idam, string iddep, EGroupBy groupby)
        {
            ...
            var ad = new KlonsF.DataSets.KlonsARepDataSetTableAdapters.SP_REP_AGGREGATETableAdapter();
            ...
            for (int i = 0; i < table.Rows.Count; i++)
            {
                ...
                switch (groupby)
                {
                    case EGroupBy.YearAndMonth:
                        ...
                        break;
                    case EGroupBy.Person:
                        var dr_person = MyData.DataSetKlonsA.PERSONS.FindByID(dr.IDP);
                        reprow.Caption = dr_person.YNAME;
                        break;
                    case EGroupBy.PersonAndPosition:
                        dr_person = MyData.DataSetKlonsA.PERSONS.FindByID(dr.IDP);
                        var dr_position = MyData.DataSetKlonsA.POSITIONS.FindByID(dr.IDAM);
                        reprow.Caption = $"{dr_person.YNAME}, {dr_position.TITLE}";
                        break;
                    case EGroupBy.Department:
                        var dr_dep = MyData.DataSetKlonsA.DEPARTMENTS.FindByID(dr.IDDEP);
